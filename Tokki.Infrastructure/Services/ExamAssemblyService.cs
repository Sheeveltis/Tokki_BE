            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                ...
                else
                {
                    ...
                    // Rút ngắn tên template để tránh lỗi quá dài (Max 256 chars)
                    string templateName = $"[WEEKLY] {examTypeLabel} | {targetTypes.Count} types | {DateTime.UtcNow:yyyyMMddHHmm}";
                    ...
                    {
                        _logger.LogError("[EXAM_ASM] Lỗi tạo Template: {Msg}", templateResult.Message);
                    ...
                    {
                        _logger.LogError("[EXAM_ASM] Lỗi tạo Parts cho Template {TemplateId}: {Msg}", newTemplateId, addPartsResult.Message);
                ...
                {
                    _logger.LogError(
                        "[EXAM_ASM] Tạo weekly exam tuần {Week} thất bại: {Msg}", weekIndex, examResult.Message);
                }
            ...
            {
                _logger.LogError(ex, "[EXAM_ASM] Lỗi hệ thống khi tạo đề thi tuần {WeekIndex}", weekIndex);
                return OperationResult<string>.Failure($"Lỗi hệ thống khi tạo đề thi: {ex.Message}", 500);
            }
        }
