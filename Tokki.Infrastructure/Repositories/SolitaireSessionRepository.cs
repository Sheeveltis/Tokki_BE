...
{
    public class SolitaireSessionRepository : ISolitaireSessionRepository
    {
        private readonly TokkiDbContext _dbContext;

        public SolitaireSessionRepository(TokkiDbContext dbContext)
        {
            _dbContext = dbContext;
        }

        public async Task<GameMatchSession?> GetByUserGameAsync(
            string userId,
            string gameId,
        ...
        public async Task<(IReadOnlyList<GameMatchSession> Items, int TotalCount)> GetPagedByGameAsync(
            string gameId,
            GameDifficulty difficulty,
            int pageNumber,
            int pageSize)
