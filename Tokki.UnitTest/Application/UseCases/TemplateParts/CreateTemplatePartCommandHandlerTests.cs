...
namespace Tokki.UnitTest.Application.UseCases.TemplateParts
{
    public class CreateTemplatePartCommandHandlerTests
    {
        ...
        // CreateTemplatePart_01 | A | ExamTemplate not found → failure
        [Fact]
        public async Task Handle_ExamTemplateNotFound_ShouldReturnFailure()
        {
            ...
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_01", Description = "ExamTemplate not found → failure (ExamTemplateNotFound)", ExpectedResult = "IsSuccess=false", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetByIdAsync returns null" } });
        }

        // CreateTemplatePart_02 | A | QuestionFrom > QuestionTo → invalid range failure
        [Fact]
        public async Task Handle_InvalidRange_QuestionFromGreaterThanTo_ShouldReturnFailure()
        {
            ...
            result.IsSuccess.Should().BeFalse();
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_02", Description = "QuestionFrom(10) > QuestionTo(5) → TemplatePartInvalidRange", ExpectedResult = "IsSuccess=false", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "QuestionFrom > QuestionTo guard" } });
        }

        // CreateTemplatePart_03 | A | QuestionFrom <= 0 → invalid range failure
        [Fact]
        public async Task Handle_QuestionFromZeroOrNegative_ShouldReturnFailure()
        {
            ...
            result.IsSuccess.Should().BeFalse();
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_03", Description = "QuestionFrom=0 → TemplatePartInvalidRange", ExpectedResult = "IsSuccess=false", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "QuestionFrom<=0 guard" } });
        }

        // CreateTemplatePart_04 | A | Question range overlaps existing part → failure
        [Fact]
        public async Task Handle_RangeOverlap_ShouldReturnFailure()
        {
            ...
            result.IsSuccess.Should().BeFalse();
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_04", Description = "Range overlaps existing part → TemplatePartRangeOverlap", ExpectedResult = "IsSuccess=false", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "IsQuestionRangeOverlapAsync returns true" } });
        }

        // CreateTemplatePart_05 | N | Happy path: valid part → 201 with generated ID
        [Fact]
        public async Task Handle_ValidRequest_ShouldReturn201WithGeneratedId()
        {
            ...
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_05", Description = "Valid part → 201, Data='TP-NEWID'", ExpectedResult = "IsSuccess=true, 201, Data='TP-NEWID'", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "All guards pass, part created" } });
        }

        // CreateTemplatePart_06 | B | AddAsync and SaveChangesAsync both called once on success
        [Fact]
        public async Task Handle_ValidRequest_AddAndSaveBothCalledOnce()
        {
            ...
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_06", Description = "AddAsync and SaveChangesAsync both called once", ExpectedResult = "Both Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Persist calls verified" } });
        }
        // CreateTemplatePart_07 | A | Skill mismatch mapped QuestionType → failure
        [Fact]
        public async Task Handle_MismatchedSkillQuestionType_ShouldReturnFailure()
        {
            ...
            QACollector.LogTestCase("TemplatePart - Create", new TestCaseDetail { FunctionGroup = "CreateTemplatePart", TestCaseID = "CreateTemplatePart_07", Description = "Skill mismatch for QuestionType", ExpectedResult = "IsSuccess=false, Contains skill mismatch message", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "questionType.Skill != request.Skill" } });
        }

        // CreateTemplatePart_08 | A | Repository throws exception → 500
        [Fact]
