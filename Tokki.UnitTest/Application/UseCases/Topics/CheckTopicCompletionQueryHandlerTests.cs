...
namespace Tokki.UnitTest.Application.UseCases.Topics
{
    public class CheckTopicCompletionQueryHandlerTests
    {
        ...
        // CheckTopicCompletion_01 | A | Topic not found → 404
        [Fact]
        ...
        public async Task Handle_TopicWithNoVocabs_ShouldReturnCompletedWith100Percent()
        {
            ...
            QACollector.LogTestCase("Topic - Check Completion", new TestCaseDetail { FunctionGroup = "CheckTopicCompletion", TestCaseID = "CheckTopicCompletion_02", Description = "No vocabs → IsCompleted=true, Progress=100", ExpectedResult = "IsCompleted=true, ProgressPercent=100", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "totalVocab=0 → complete" } });
        }

        // CheckTopicCompletion_03 | N | 5/10 vocabs learned → Progress=50, IsCompleted=false
        [Fact]
        public async Task Handle_5Of10Learned_ShouldReturn50PercentNotCompleted()
        {
            ...
            QACollector.LogTestCase("Topic - Check Completion", new TestCaseDetail { FunctionGroup = "CheckTopicCompletion", TestCaseID = "CheckTopicCompletion_03", Description = "5/10 learned → ProgressPercent=50, IsCompleted=false", ExpectedResult = "ProgressPercent=50, IsCompleted=false", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "learned/total=50%" } });
        }

        // CheckTopicCompletion_04 | N | All 10/10 vocabs learned → IsCompleted=true, Progress=100
        [Fact]
        public async Task Handle_AllLearned_ShouldReturnCompletedWith100Percent()
        {
            ...
            result.Data!.IsCompleted.Should().BeTrue();
            result.Data.ProgressPercent.Should().Be(100);
            QACollector.LogTestCase("Topic - Check Completion", new TestCaseDetail { FunctionGroup = "CheckTopicCompletion", TestCaseID = "CheckTopicCompletion_04", Description = "10/10 learned → IsCompleted=true, Progress=100", ExpectedResult = "IsCompleted=true, ProgressPercent=100", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "learnedCount>=totalVocab" } });
        }

        // CheckTopicCompletion_05 | N | TopicId echoed in DTO
        [Fact]
        public async Task Handle_TopicFound_TopicIdEchoedInDto()
        {
            ...
            QACollector.LogTestCase("Topic - Check Completion", new TestCaseDetail { FunctionGroup = "CheckTopicCompletion", TestCaseID = "CheckTopicCompletion_05", Description = "TopicId='T-001' echoed in result DTO", ExpectedResult = "Data.TopicId='T-001'", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "TopicId mapped to DTO" } });
        }

        // CheckTopicCompletion_06 | B | CountLearnedVocabulariesAsync called with correct userId and topicId
        [Fact]
        public async Task Handle_TopicFound_CountLearnedCalledWithCorrectIds()
        {
            ...
            QACollector.LogTestCase("Topic - Check Completion", new TestCaseDetail { FunctionGroup = "CheckTopicCompletion", TestCaseID = "CheckTopicCompletion_06", Description = "CountLearnedVocabulariesAsync called with userId='U-XYZ', topicId='T-001'", ExpectedResult = "Times.Once with correct IDs", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Both IDs forwarded to repo" } });
        }
    }
}