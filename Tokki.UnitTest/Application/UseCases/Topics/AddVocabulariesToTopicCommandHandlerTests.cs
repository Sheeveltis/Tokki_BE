...
namespace Tokki.UnitTest.Application.UseCases.Topics
{
    public class AddVocabulariesToTopicCommandHandlerTests
    {
        ...
        // TC-TOPIC-ADDV-01 | A | Validation fails → 400
        [Fact]
        public async Task Handle_ValidationFails_ShouldReturn400()
        {
            ...
            QACollector.LogTestCase("Topic - Add Vocabularies", new TestCaseDetail { FunctionGroup = "AddVocabulariesToTopic", TestCaseID = "TC-TOPIC-ADDV-01", Description = "Validation fails → 400", ExpectedResult = "IsSuccess=false, 400", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "FluentValidation returns errors" } });
        }

        // TC-TOPIC-ADDV-02 | A | Topic not found → 404
        [Fact]
        public async Task Handle_TopicNotFound_ShouldReturn404()
        {
            ...
            QACollector.LogTestCase("Topic - Add Vocabularies", new TestCaseDetail { FunctionGroup = "AddVocabulariesToTopic", TestCaseID = "TC-TOPIC-ADDV-02", Description = "Topic not found → 404", ExpectedResult = "IsSuccess=false, 404", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetByIdAsync returns null" } });
        }

        // TC-TOPIC-ADDV-03 | A | No valid vocabularies found → 400
        [Fact]
        public async Task Handle_NoValidVocabs_ShouldReturn400()
        {
            ...
            result.IsSuccess.Should().BeFalse();
            result.StatusCode.Should().Be(400);
            QACollector.LogTestCase("Topic - Add Vocabularies", new TestCaseDetail { FunctionGroup = "AddVocabulariesToTopic", TestCaseID = "TC-TOPIC-ADDV-03", Description = "No valid vocabs found → 400", ExpectedResult = "IsSuccess=false, 400", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetByIdsAsync returns empty list" } });
        }

        // TC-TOPIC-ADDV-04 | N | Happy path: 1 vocab added → 200 with addedCount=1
        [Fact]
        ...
        public async Task Handle_ValidRequest_AddOrReactivateCalled()
        {
            ...
            QACollector.LogTestCase("Topic - Add Vocabularies", new TestCaseDetail { FunctionGroup = "AddVocabulariesToTopic", TestCaseID = "TC-TOPIC-ADDV-05", Description = "AddOrReactivate called with correct topicId and vocabs", ExpectedResult = "Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Repo called once" } });
        }

        // TC-TOPIC-ADDV-06 | N | Duplicate vocab IDs in request deduplicated
        [Fact]
