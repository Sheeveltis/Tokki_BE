...
namespace Tokki.UnitTest.Application.UseCases.Topics
{
    public class GetAllTopicsQueryHandlerTests
    {
        ...
        private static GetAllTopicsQueryHandler CreateHandler(Mock<ITopicRepository>? repo = null)
            => new GetAllTopicsQueryHandler((repo ?? GetRepoMock()).Object);
        ...
        public async Task Handle_ReturnsTopics_DtoFieldsMappedCorrectly()
        {
            ...
            QACollector.LogTestCase("Topic - Get All", new TestCaseDetail { FunctionGroup = "GetAllTopics", TestCaseID = "TC-TOPIC-GALL-02", Description = "DTO fields: Id, Name, Status, VocabularyCount=5 mapped", ExpectedResult = "All DTO fields correct", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "CountVocabulariesInTopicAsync returns 5" } });
        }

        // TC-TOPIC-GALL-03 | B | GetVocabTopicsPagedAsync called with query params
        [Fact]
        ...
        // TC-TOPIC-GALL-04 | N | Empty repo → 200 with empty Items
        [Fact]
        public async Task Handle_NoTopics_ShouldReturn200WithEmptyItems()
        {
            ...
            QACollector.LogTestCase("Topic - Get All", new TestCaseDetail { FunctionGroup = "GetAllTopics", TestCaseID = "TC-TOPIC-GALL-04", Description = "No topics → 200 with empty paged result", ExpectedResult = "Items=[], TotalCount=0", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "No topics in system" } });
        }

        // TC-TOPIC-GALL-05 | N | Paging metadata correct
        [Fact]
        public async Task Handle_Page3Size5Total50_PagingMetadataCorrect()
        {
            ...
            QACollector.LogTestCase("Topic - Get All", new TestCaseDetail { FunctionGroup = "GetAllTopics", TestCaseID = "TC-TOPIC-GALL-05", Description = "Page=3, Size=5, TotalCount=50 paging metadata correct", ExpectedResult = "Metadata correct", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Paging values in PagedResult" } });
        }

        // TC-TOPIC-GALL-06 | B | CountVocabulariesInTopicAsync called once per topic
        [Fact]
