...
namespace Tokki.UnitTest.Application.UseCases.StatisticBlog
{
    public class GetTopBlogsQueryHandlerTests
    {
        ...
        // GetTopBlogs_01 | N | Happy path: 2 blogs returned → 200 with Count=2
        [Fact]
        public async Task Handle_RepoReturnsBlogs_ShouldReturn200WithList()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Blogs", new TestCaseDetail { FunctionGroup = "GetTopBlogs", TestCaseID = "GetTopBlogs_01", Description = "2 blogs returned → 200, Count=2", ExpectedResult = "IsSuccess=true, Data.Count=2", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTopViewedBlogsAsync returns 2 items" } });
        }

        // GetTopBlogs_02 | N | Blog fields Title and ViewCount mapped correctly
        [Fact]
        public async Task Handle_RepoReturnsBlogs_FieldsMappedCorrectly()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Blogs", new TestCaseDetail { FunctionGroup = "GetTopBlogs", TestCaseID = "GetTopBlogs_02", Description = "Blog Title='Grammar Guide', ViewCount=999 mapped correctly", ExpectedResult = "All fields verified", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Blog fields passed directly" } });
        }

        // GetTopBlogs_03 | B | GetTopViewedBlogsAsync called with correct Count
        [Fact]
        public async Task Handle_WithCount10_GetTopViewedBlogsCalledWithCount10()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Blogs", new TestCaseDetail { FunctionGroup = "GetTopBlogs", TestCaseID = "GetTopBlogs_03", Description = "GetTopViewedBlogsAsync called with Count=10", ExpectedResult = "Times.Once with count=10", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Count=10 passed correctly" } });
        }

        // GetTopBlogs_04 | B | Default Count=5 when not specified
        [Fact]
        public async Task Handle_DefaultCount_ShouldCallRepoWithFive()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Blogs", new TestCaseDetail { FunctionGroup = "GetTopBlogs", TestCaseID = "GetTopBlogs_04", Description = "Default Count=5 → repo called with 5", ExpectedResult = "GetTopViewedBlogsAsync(5, ...) Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTopBlogsQuery().Count=5 by default" } });
        }

        // GetTopBlogs_05 | N | Empty list → 200 with empty data
        [Fact]
        public async Task Handle_NoBlogs_ShouldReturn200WithEmptyList()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Blogs", new TestCaseDetail { FunctionGroup = "GetTopBlogs", TestCaseID = "GetTopBlogs_05", Description = "No blogs → 200 with empty list", ExpectedResult = "IsSuccess=true, Data=[]", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Repo returns [] (no blogs)", "still 200" } });
        }

        // GetTopBlogs_06 | A | Repository throws → exception propagates
        [Fact]
