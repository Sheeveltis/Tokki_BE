...
namespace Tokki.UnitTest.Application.UseCases.StatisticBlog
{
    public class GetTopAuthorsQueryHandlerTests
    {
        ...
        private static Mock<IStatisticBlogRepository> GetRepoMock(List<TopAuthorDTO>? data = null)
        {
            ...
            m.Setup(x => x.GetTopAuthorsAsync(It.IsAny<int>(), It.IsAny<AuthorSource>(), It.IsAny<CancellationToken>()))
        ...
        // GetTopAuthors_01 | N | Happy path: repo returns 3 authors → 200 with 3 items
        [Fact]
        public async Task Handle_RepoReturnsAuthors_ShouldReturn200WithList()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_01", Description = "Happy path: 3 authors returned → 200, Count=3", ExpectedResult = "IsSuccess=true, Data.Count=3", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTopAuthorsAsync returns 3 items" } });
        }

        // GetTopAuthors_02 | N | Author fields mapped correctly (AuthorId, TotalViews, BlogCount)
        [Fact]
        public async Task Handle_RepoReturnsAuthors_FieldsMappedCorrectly()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_02", Description = "Author fields mapped correctly", ExpectedResult = "AuthorId='A99', BlogCount=25", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Author fields passed directly" } });
        }

        // GetTopAuthors_03 | B | GetTopAuthorsAsync called with correct Count parameter
        [Fact]
        public async Task Handle_WithCount5_GetTopAuthorsCalledWithCount5()
        {
            ...
            repo.Verify(x => x.GetTopAuthorsAsync(5, It.IsAny<AuthorSource>(), It.IsAny<CancellationToken>()), Times.Once);
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_03", Description = "GetTopAuthorsAsync called with Count=5", ExpectedResult = "Times.Once with count=5", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Count=5 passed correctly to repo" } });
        }

        // GetTopAuthors_04 | B | Default Count=5 used when not specified
        [Fact]
        public async Task Handle_DefaultCount_ShouldCallRepoWithFive()
        {
            ...
            repo.Verify(x => x.GetTopAuthorsAsync(5, It.IsAny<AuthorSource>(), It.IsAny<CancellationToken>()), Times.Once);
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_04", Description = "Default Count=5 → repo called with 5", ExpectedResult = "GetTopAuthorsAsync(5, ...) Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTopAuthorsQuery().Count = 5 by default" } });
        }

        // GetTopAuthors_05 | N | Empty list → 200 with empty data
        [Fact]
        public async Task Handle_NoAuthors_ShouldReturn200WithEmptyList()
        {
            ...
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_05", Description = "No authors in system → 200 with empty list", ExpectedResult = "IsSuccess=true, Data=[]", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Repo returns empty list", "no authors yet" } });
        }

        // GetTopAuthors_06 | A | Repository throws → exception propagates
        [Fact]
        public async Task Handle_RepoThrows_ShouldPropagateException()
        {
            var repo = new Mock<IStatisticBlogRepository>();
            repo.Setup(x => x.GetTopAuthorsAsync(It.IsAny<int>(), It.IsAny<AuthorSource>(), It.IsAny<CancellationToken>()))
                .ThrowsAsync(new Exception("DB error"));
            var act = async () => await CreateHandler(repo).Handle(new GetTopAuthorsQuery(), CancellationToken.None);
            await act.Should().ThrowAsync<Exception>().WithMessage("DB error");
            QACollector.LogTestCase("StatisticBlog - Top Authors", new TestCaseDetail { FunctionGroup = "GetTopAuthors", TestCaseID = "GetTopAuthors_06", Description = "Repository throws → exception propagates", ExpectedResult = "Exception thrown", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTopAuthorsAsync throws" } });
        }
    }
}
