...
namespace Tokki.UnitTest.Application.UseCases.Games
{
    public class SaveGameResultCommandHandlerTests
    {
        ...
        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-01 | 401 | No userId in token → Unauthorized
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
        public async Task Handle_NoUserIdInToken_ShouldReturn401()
        {
            // Arrange
            var httpCtx = BuildHttpCtx(null);
        ...
        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-02 | 404 | Game not found or inactive → GameNotFound
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
        public async Task Handle_GameNotFound_ShouldReturn404()
        {
            // Arrange
            var httpCtx = BuildHttpCtx("USER-001");

            var mockGameRepo = new Mock<IGameRepository>();
            mockGameRepo.Setup(x => x.GetByIdAsync(It.IsAny<string>()))
                        .ReturnsAsync((Game?)null);

            // Act
            var result = await CreateHandler(gameRepo: mockGameRepo, httpCtx: httpCtx)
                             .Handle(ValidCommand, CancellationToken.None);
            ...
            QACollector.LogTestCase("Games - Save Result", new TestCaseDetail
            {
                ...
                TestCaseType      = "A",
                TestDate          = DateTime.Now.ToString("dd/MM/yyyy"),
                AppliedConditions = new List<string> { "game == null || game.Status != Active" }
            });
        }

        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-03 | 404 | Game is inactive → GameNotFound
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
        public async Task Handle_GameInactive_ShouldReturn404()
        {
            ...
            QACollector.LogTestCase("Games - Save Result", new TestCaseDetail
            {
                ...
                Description       = "Game.Status = Inactive → GameNotFound error",
                ExpectedResult    = "Return 404 GameNotFound",
                StatusRound1      = "Passed",
                TestCaseType      = "N",
                TestDate          = DateTime.Now.ToString("dd/MM/yyyy"),
        ...
        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-04 | 200 | Session null → new session created and saved
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
        ...
        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-05 | 200 | Existing session + higher score → BestScore updated
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
        public async Task Handle_ExistingSession_HigherScore_ShouldUpdateBestScore()
        {
            ...
            // Act
            var result = await CreateHandler(
                gameRepo: mockGameRepo, sessionRepo: mockSessionRepo, httpCtx: httpCtx
        ...
        // ═══════════════════════════════════════════════════════════════════
        // TC-GAME-SGR-06 | 500 | Repository throws → ServerError
        // ═══════════════════════════════════════════════════════════════════
        [Fact]
