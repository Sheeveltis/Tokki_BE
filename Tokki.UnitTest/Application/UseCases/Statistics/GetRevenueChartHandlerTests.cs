...
namespace Tokki.UnitTest.Application.UseCases.Statistics
{
    public class GetRevenueChartHandlerTests
    {
        ...
        // TC-STAT-RC-01 | N | Happy path: returns monthly chart data → success
        [Fact]
        public async Task Handle_RepoReturnsList_ShouldReturnSuccessWithData()
        {
            ...
            QACollector.LogTestCase("Statistics - Revenue Chart", new TestCaseDetail { FunctionGroup = "GetRevenueChart", TestCaseID = "TC-STAT-RC-01", Description = "3 months data returned → success, Count=3", ExpectedResult = "IsSuccess=true, Data.Count=3", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetRevenueChartAsync returns 3 items" } });
        }

        // TC-STAT-RC-02 | N | RevenueChartDto fields correctly mapped
        [Fact]
        public async Task Handle_ReturnsData_MonthRevenueTotalOrdersMapped()
        {
            ...
            QACollector.LogTestCase("Statistics - Revenue Chart", new TestCaseDetail { FunctionGroup = "GetRevenueChart", TestCaseID = "TC-STAT-RC-02", Description = "Month='2024-05', Revenue=7777, TotalOrders=42 mapped correctly", ExpectedResult = "All fields correct", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "DTO fields passed through" } });
        }

        // TC-STAT-RC-03 | B | GetRevenueChartAsync called with correct Year
        [Fact]
        public async Task Handle_WithYear2023_RepoCalledWithYear2023()
        {
            ...
            QACollector.LogTestCase("Statistics - Revenue Chart", new TestCaseDetail { FunctionGroup = "GetRevenueChart", TestCaseID = "TC-STAT-RC-03", Description = "GetRevenueChartAsync called with Year=2023", ExpectedResult = "Times.Once with year=2023", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Year parameter forwarded correctly" } });
        }

        // TC-STAT-RC-04 | N | Empty list (no data for year) → 200 with empty list
        [Fact]
        public async Task Handle_NoDataForYear_ShouldReturn200WithEmptyList()
        {
            ...
            QACollector.LogTestCase("Statistics - Revenue Chart", new TestCaseDetail { FunctionGroup = "GetRevenueChart", TestCaseID = "TC-STAT-RC-04", Description = "No data for year → 200 with empty list", ExpectedResult = "IsSuccess=true, Data=[]", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Year with no records" } });
        }

        // TC-STAT-RC-05 | N | Data reference from repo preserved
        [Fact]
        ...
        public async Task Handle_RepoThrows_ShouldPropagateException()
        {
            ...
            QACollector.LogTestCase("Statistics - Revenue Chart", new TestCaseDetail { FunctionGroup = "GetRevenueChart", TestCaseID = "TC-STAT-RC-06", Description = "Repository throws → exception propagates", ExpectedResult = "Exception thrown", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetRevenueChartAsync throws" } });
        }
    }
}
