...
namespace Tokki.UnitTest.Application.UseCases.Roadmap
{
    public class CancelRoadmapCommandHandlerTests
    {
        ...
        public async Task Handle_NoActiveRoadmap_ShouldReturn404()
        {
            ...
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_01", Description = "No active roadmap → 404", ExpectedResult = "IsSuccess=false, 404", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetActiveRoadmapByUserIdAsync returns null" } });
        }

        // CancelRoadmap_02 | N | Active roadmap exists → status set to Dropped, success
        [Fact]
        public async Task Handle_ActiveRoadmap_ShouldSetDroppedAndReturn200()
        {
            ...
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_02", Description = "Active roadmap → Dropped, 200", ExpectedResult = "IsSuccess=true, Status=Dropped", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Active roadmap found", "Status set to Dropped" } });
        }

        // CancelRoadmap_03 | B | SaveChangesAsync called once after status change
        [Fact]
        public async Task Handle_ActiveRoadmap_SaveChangesCalledOnce()
        {
            ...
            repo.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Once);
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_03", Description = "Boundary: SaveChangesAsync called exactly once", ExpectedResult = "SaveChangesAsync Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "SaveChangesAsync called once" } });
        }

        // CancelRoadmap_04 | A | Repository throws → exception propagates
        [Fact]
        public async Task Handle_RepositoryThrows_ShouldPropagateException()
        {
            ...
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_04", Description = "SaveChangesAsync throws → exception propagates", ExpectedResult = "Exception thrown", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "SaveChangesAsync throws" } });
        }

        // CancelRoadmap_05 | N | UserId is passed correctly to GetActiveRoadmapByUserIdAsync
        [Fact]
        public async Task Handle_ValidUser_GetActiveRoadmapCalledWithCorrectUserId()
        {
            ...
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_05", Description = "GetActiveRoadmapByUserIdAsync called with correct UserId", ExpectedResult = "GetActiveRoadmapByUserIdAsync('USER-XYZ') Once", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "UserId passed correctly" } });
        }

        // CancelRoadmap_06 | N | Status is specifically Dropped (not just changed)
        [Fact]
        public async Task Handle_CancelRoadmap_StatusIsExactlyDropped()
        {
            ...
            await CreateHandler(repo).Handle(new CancelRoadmapCommand { UserId = roadmap.UserId }, CancellationToken.None);
            roadmap.CurrentStatus.Should().Be(UserRoadmapStatus.Dropped);
            QACollector.LogTestCase("Roadmap - Cancel", new TestCaseDetail { FunctionGroup = "CancelRoadmap", TestCaseID = "CancelRoadmap_06", Description = "Cancelled roadmap status is exactly UserRoadmapStatus.Dropped", ExpectedResult = "CurrentStatus=Dropped", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "UserRoadmapStatus.Dropped applied" } });
        }
    }
}
