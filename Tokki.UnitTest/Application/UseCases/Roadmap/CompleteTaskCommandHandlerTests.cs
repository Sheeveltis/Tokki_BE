...
namespace Tokki.UnitTest.Application.UseCases.Roadmap
{
    public class CompleteTaskCommandHandlerTests
    {
        ...
        // CompleteTask_01 | A | Task not found → 404
        [Fact]
        public async Task Handle_TaskNotFound_ShouldReturn404()
        {
            ...
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_01", Description = "Task not found → 404", ExpectedResult = "IsSuccess=false, 404", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetTaskByIdAsync returns null" } });
        }

        // CompleteTask_02 | A | Task belongs to different user → 403
        [Fact]
        public async Task Handle_DifferentUser_ShouldReturn403()
        {
            ...
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_02", Description = "Wrong user → 403, SaveChanges never called", ExpectedResult = "IsSuccess=false, 403", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "UserId mismatch" } });
        }

        // CompleteTask_03 | N | Task not yet completed → marked as completed, saved, success
        [Fact]
        public async Task Handle_IncompleteTask_ShouldMarkCompleteAndSave()
        {
            ...
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_03", Description = "Incomplete task → IsCompleted=true, saved, success", ExpectedResult = "IsSuccess=true, task.IsCompleted=true", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "task.IsCompleted was false → set true" } });
        }

        // CompleteTask_04 | N | Already completed task → no SaveChanges call (idempotent)
        [Fact]
        public async Task Handle_AlreadyCompletedTask_ShouldNotSaveAgain()
        {
            ...
            result.IsSuccess.Should().BeTrue();
            repo.Verify(x => x.SaveChangesAsync(It.IsAny<CancellationToken>()), Times.Never);
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_04", Description = "Already completed task → SaveChanges not called (idempotent)", ExpectedResult = "IsSuccess=true, SaveChanges Times.Never", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "task.IsCompleted already true", "SaveChanges skipped" } });
        }

        // CompleteTask_05 | B | GetTaskByIdAsync called once with correct TaskId
        [Fact]
        public async Task Handle_ValidCommand_GetTaskByIdCalledWithCorrectId()
        {
            ...
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_05", Description = "Boundary: GetTaskByIdAsync called with exact TaskId", ExpectedResult = "GetTaskByIdAsync('TASK-SPECIFIC-01') Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "TaskId matches exactly" } });
        }

        // CompleteTask_06 | A | Repository SaveChanges throws → exception propagates
        [Fact]
        public async Task Handle_SaveThrows_ShouldPropagateException()
        {
            ...
            QACollector.LogTestCase("Roadmap - Complete Task", new TestCaseDetail { FunctionGroup = "CompleteTask", TestCaseID = "CompleteTask_06", Description = "SaveChangesAsync throws → exception propagates", ExpectedResult = "Exception thrown", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "SaveChangesAsync throws" } });
        }
    }
}
