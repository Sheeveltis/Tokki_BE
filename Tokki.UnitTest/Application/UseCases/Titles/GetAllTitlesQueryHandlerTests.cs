...
namespace Tokki.UnitTest.Application.UseCases.Titles
{
    public class GetAllTitlesQueryHandlerTests
    {
        ...
        // GetAllTitles_01 | N | Happy path: 3 titles returned → 200 with Count=3
        [Fact]
        public async Task Handle_RepoReturns3Titles_ShouldReturn200With3Items()
        {
            ...
            QACollector.LogTestCase("Title - Get All", new TestCaseDetail { FunctionGroup = "GetAllTitles", TestCaseID = "GetAllTitles_01", Description = "3 titles → 200, Count=3", ExpectedResult = "IsSuccess=true, 200, Data.Count=3", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "GetAllTitlesAsync returns 3 items" } });
        }

        // GetAllTitles_02 | N | Empty list → 200 with empty list
        [Fact]
        public async Task Handle_NoTitles_ShouldReturn200WithEmptyList()
        {
            ...
            QACollector.LogTestCase("Title - Get All", new TestCaseDetail { FunctionGroup = "GetAllTitles", TestCaseID = "GetAllTitles_02", Description = "No titles → 200 with empty list", ExpectedResult = "IsSuccess=true, Data=[]", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "No titles in system" } });
        }

        // GetAllTitles_03 | B | GetAllTitlesAsync called exactly once
        [Fact]
        public async Task Handle_CallsGetAllTitlesAsync_ExactlyOnce()
        {
            ...
            QACollector.LogTestCase("Title - Get All", new TestCaseDetail { FunctionGroup = "GetAllTitles", TestCaseID = "GetAllTitles_03", Description = "GetAllTitlesAsync called exactly once", ExpectedResult = "Times.Once", StatusRound1 = "Passed", TestCaseType = "B", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Single repo call" } });
        }

        // GetAllTitles_04 | N | First title fields correctly returned (TitleId, Name, RequiredXP)
        [Fact]
        public async Task Handle_ReturnsData_FirstTitleFieldsCorrect()
        {
            ...
            QACollector.LogTestCase("Title - Get All", new TestCaseDetail { FunctionGroup = "GetAllTitles", TestCaseID = "GetAllTitles_04", Description = "First title: TitleId='T-001', Name='Bậc học giả', XP=1000", ExpectedResult = "All fields correct", StatusRound1 = "Passed", TestCaseType = "N", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "Title entity returned directly" } });
        }

        // GetAllTitles_05 | N | Data is same reference list as returned by repo
        [Fact]
        ...
        public async Task Handle_RepoThrows_ShouldPropagateException()
        {
            ...
            QACollector.LogTestCase("Title - Get All", new TestCaseDetail { FunctionGroup = "GetAllTitles", TestCaseID = "GetAllTitles_06", Description = "Repository throws → exception propagates (no try-catch)", ExpectedResult = "Exception thrown", StatusRound1 = "Passed", TestCaseType = "A", TestDate = DateTime.Now.ToString("dd/MM/yyyy"), AppliedConditions = new List<string> { "No exception handling in handler" } });
        }
    }
}
