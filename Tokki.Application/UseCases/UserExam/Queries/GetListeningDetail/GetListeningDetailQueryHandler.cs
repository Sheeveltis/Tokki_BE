...
{
    public class GetListeningDetailQueryHandler : IRequestHandler<GetListeningDetailQuery, OperationResult<ListeningDetailResponse>>
    {
        private readonly IUserExamRepository _repository;

        public GetListeningDetailQueryHandler(IUserExamRepository repository)
        {
            _repository = repository;
        }

        public async Task<OperationResult<ListeningDetailResponse>> Handle(GetListeningDetailQuery request, CancellationToken token)
        {
            var session = await _repository.GetListeningDetailAsync(request.UserExamId, token);
